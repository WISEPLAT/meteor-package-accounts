...
@module Wiseplat:accounts
*/



/**
The accounts collection, with some wiseplat additions.

@class WshAccounts
@constructor
*/
var collection = new Mongo.Collection('wiseplat_accounts', {connection: null});
WshAccounts = _.clone(collection);
WshAccounts._collection = collection;


if(typeof PersistentMinimongo !== 'undefined')
    new PersistentMinimongo(WshAccounts._collection);
...
*/
WshAccounts._watchBalance = function(){
    var _this = this;
    ...
    this.blockSubscription = web3.wsh.filter('latest');
...
*/
WshAccounts._updateBalance = function(){
    var _this = this;

    _.each(WshAccounts.find({}).fetch(), function(account){
        web3.wsh.getBalance(account.address, function(err, res){
            if(!err) {
                if(res.toFixed) {
                    res = res.toFixed();
                }

                WshAccounts.update(account._id, {
                    $set: {
                        balance: res
                    }
                });
...
@method _addAccounts
*/
WshAccounts._addAccounts = function(){
    var _this = this;

    // UPDATE normal accounts on start
    web3.wsh.getAccounts(function(e, accounts){
        if(!e) {
            var visibleAccounts = _.pluck(WshAccounts.find().fetch(), 'address');
            ...
            var localAccounts = WshAccounts.findAll().fetch();
            ...
            _.each(localAccounts, function(account){
                ...
                if(!_.contains(accounts, account.address)) {
                    WshAccounts.updateAll(account._id, {
                        $set: {
                            deactivated: true
                        }
                    });
            ...
            _.each(accounts, function(address){
                ...
                web3.wsh.getBalance(address, function(e, balance){
                    if(!e) {
                        if(balance.toFixed) {
                            balance = balance.toFixed();
                        }

                        web3.wsh.getCoinbase(function(e, coinbase){
                            var doc = WshAccounts.findAll({
                                address: address,
                            }).fetch()[0];
                            ...
                            var insert = {
                                ...
                                address: address,
                                balance: balance,
                                name: (address === coinbase) ? 'Main account (Wisebase)' : 'Account '+ accountsCount
                            };

                            if(doc) {
                                WshAccounts.updateAll(doc._id, {
                                    $set: insert
                                });
                            } else {
                                WshAccounts.insert(insert);
                            }
...
*/
WshAccounts._addToQuery = function(args, options){
    var _this = this;
...
@return {Object} cursor
*/
WshAccounts.updateAll = function() {
    return this._collection.update.apply(this, this._addToQuery(arguments, {
        includeDeactivated: true
    }));
}
...
@method init
*/
WshAccounts.init = function() {
    var _this = this;

    if(typeof web3 === 'undefined') {
        console.warn('WshAccounts couldn\'t find web3, please make sure to instantiate a web3 object before calling WshAccounts.init()');
        return;
    }
